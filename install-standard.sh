...
function create_database {
  if [ "$OS" == "centos" ] || [ "$OS" == "almalinux" ]; then
    ...
    mariadb -u root -p -e "CREATE USER '${MYSQL_USER}'@'127.0.0.1' IDENTIFIED BY '${PASSWORD}';"
    ...
    mariadb -u root -e "CREATE USER '${MYSQL_USER}'@'127.0.0.1' IDENTIFIED BY '${PASSWORD}';"
...
function debian_stretch_dep {
  ...
  DEBIAN_FRONTEND=noninteractive apt install -y ca-certificates apt-transport-https lsb-release jq
